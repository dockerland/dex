...
DOCKER_GROUP ?= docker
DOCKER_GID ?= $(shell if type getent >/dev/null 2>&1; then getent group $(DOCKER_GROUP) | cut -d: -f3 ; elif type dscl >/dev/null 2>&1; then dscl . -read /Groups/$(DOCKER_GROUP) PrimaryGroupID 2>/dev/null | awk '{ print $2 }' ; else python -c "import grp; print(grp.getgrnam(\"$(DOCKER_GROUP)\").gr_gid)" 2>/dev/null ; fi)
